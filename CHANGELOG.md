...
## Fork additions on top of `ajl-agglayer-integration-tests-v0.14.0`

### Enhancements

* [FEATURE][rust] Added `GrpcClient::with_header(key, value)` to inject caller-supplied request metadata (e.g. `authorization: Bearer <token>`) into every outbound gRPC call, for use behind authenticating gateways. Headers are validated at connection time and preserved across `set_genesis_commitment` updates. Backport of the upstream PR [`0xMiden/miden-client#2101`](https://github.com/0xMiden/miden-client/pull/2101); revert this branch and swap the pin back to upstream once that PR lands.
...
>>>>>>> 8e597970 (feat(rust-client): let callers inject extra gRPC request headers)
## 0.14.0 (2026-04-07)
